...
def selecionar_menu(titulo, opcoes, valor_padrao=None):
    if not sys.stdin.isatty():
        return valor_padrao or (opcoes[0][0] if isinstance(opcoes[0], tuple) else opcoes[0])
    ...
    while True:
        ...
        if tecla == "up":
            ...
            return selecionado[0] if isinstance(selecionado, tuple) else selecionado
        elif tecla in ("esc",):
            return valor_padrao or (opcoes[0][0] if isinstance(opcoes[0], tuple) else opcoes[0])
