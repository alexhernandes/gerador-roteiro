...
PALAVRAS_SANGUE = re.compile(
    r"\b(blood|sangue|bleeding|hemorrag|sangre|blut|bluten)\b|血(?:液|まみれ)?",
    re.I,
)
PALAVRAS_AGRESSAO = re.compile(
    r"\b(punch|stab|shoot|kill|murder|matar|soc[oou]|esfaquear|esfaque|agredir|agressão|agressao|"
    r"golpear|apuñalar|disparar|töten|erstechen|schießen)\b|殺(?:す|した)|刺(?:す|した)",
    re.I,
)
PALAVRAS_CLIFFHANGER = re.compile(
    ...
    r"impossível|plot twist|reviravolta|espera|continuará|quién es|qué es|"
    r"warte|fortsetzung folgt|wer ist|was ist|unmöglich)\b|待って|つづく|続く|誰|何だ|信じられない",
    re.I,
)
...
PALAVRAS_TRAMA = re.compile(
    ...
    r"plot|gancho|hook|cliffhanger|virada|traición|secreto|mentira|conflicto|crisis|"
    r"verrat|geheimnis|lüge|konflikt|krise|wendung)\b|裏切|秘密|嘘|対立|危機|真実",
    re.I,
)
PALAVRAS_HOOK = re.compile(
    r"\b(hook|gancho|grita|explod|impact|choc|revel|confront|entra|começa|comeca|"
    r"olha|escuta|pera|espera|não|nao|what|quem|como|por que|why|soco|corre|"
    r"mira|escucha|espera|warum|warte|schau|hör)\b|見て|待って|なぜ|聞いて|まさか",
    re.I,
)
PALAVRAS_CAMERA = re.compile(
    ...
    r"0-\d{1,3}s|\d{1,3}-\d{1,3}s)\b",
    re.I,
)
PALAVRAS_FILLER = re.compile(
    r"^(oi|olá|ola|hey|hi|tudo bem|e aí|e ai|bom dia|good morning|hola|buenos días|"
    r"hallo|guten morgen|ok|okay|sim|sí|ja|nein|não|nao|こんにちは|おはよう)\s*[!.?。！]*$",
    re.I,
)
...
def validar_elenco(elenco):
    ...
    regra_variedade = _buscar_regra("variedade_elenco")
    nomes = [str(p.get("name", "")).strip().casefold() for p in cast]
    tipos = [
        str(p.get("character_type") or p.get("fruit_type") or "").strip().casefold()
        for p in cast
    ]
    quantidade_ok = 3 <= len(cast) <= 5
    distintos = (
        all(nomes)
        and all(tipos)
    ...
    resultados.append(_resultado(
        regra_variedade,
        quantidade_ok and distintos,
        f"{len(cast)} personagens; {len(set(tipos))} tipos e {len(set(nomes))} nomes distintos.",
    ))
    ...
    regra_res = _buscar_regra("resolucao_video")
...
def validar_roteiro(roteiro, elenco, sinopse=None):
    ...
    # Quantidade e duracao total configuradas.
    qtd = len(cenas)
    ...
    for chave, cena in cenas_ordenadas:
        ...
        if len(camera) < 40:
            ...
            rf"\b{DURACAO_CENA}\s*(?:s|sec|seconds|segundos)\b",
            camera,
            re.I,
        ):
            resultados.append(_resultado(
                regra_camera,
                False,
                f"Cena {num}: CAMERA_DIRECTION não cobre até {DURACAO_CENA}s.",
            ))
        else:
            resultados.append(_resultado(
                regra_camera, True,
        ...
        if timing["line_count"] < MIN_FALAS_POR_CENA:
            ...
            resultados.append(_resultado(
                ...
                f"Cena {num}: {timing['line_count']} falas, {timing['word_count']} {unidade}, "
        ...
        if speakers:
            ...
            elif "CANONICAL CHARACTER LOCKS:" not in visual:
                resultados.append(_resultado(
                    regra_char,
                    False,
                    f"Cena {num}: personagens sem physical_dna/outfit_dna canonicos.",
                ))
            else:
                resultados.append(_resultado(
                    regra_char,
                    True,
                    f"Cena {num}: personagens e DNA canonico no VISUAL_PROMPT.",
                ))
