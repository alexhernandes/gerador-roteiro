...
from config import MAX_TOKENS_AUDIT
...
def auditar_roteiro(roteiro, sinopse, elenco, idioma):
    roteiro_json = json.dumps(
        _compactar_roteiro(roteiro), ensure_ascii=False, separators=(",", ":")
    )
...
"""
    ...
    try:
        resposta = chamar_api(
            system,
            user,
            schema=AUDITORIA_RESPONSE_FORMAT,
            max_completion_tokens=MAX_TOKENS_AUDIT,
        )
