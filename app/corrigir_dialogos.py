...
idioma correto e timing que preenche cada clipe.
"""

import json

from api import chamar_api, extrair_json
from config import MAX_TOKENS_DIALOGUES, MAX_TOKENS_REPAIR
from dialogo import (
    ajustar_timing_cena,
...
def _exemplo_dialogo(idioma):
    ...
    regras = {
        "pt": "Use portugues brasileiro natural, com concordancia e sem traducoes literais.",
        "en": "Use natural conversational English only.",
        "es": "Use natural neutral Spanish; do not mix Portuguese cognates.",
        "de": "Use natural German syntax, articles and inflection; never translate word by word from Portuguese.",
        "ja": "Use natural Japanese script and sentence endings; do not romanize the dialogue.",
    }
    return regras.get(codigo, regras["pt"])
...
def _chamar_correcao_api(system, user, schema, max_completion_tokens=MAX_TOKENS_REPAIR):
    ultimo_erro = None
...
LIMITES RÍGIDOS:
- {MIN_FALAS_POR_CENA}-{MAX_FALAS_POR_CENA} falas ({min_linha}-{max_linha} {unidade} cada)
- {min_palavras}-{max_palavras} {unidade} no total (~{MIN_SEGUNDOS_FALA}-{MAX_SEGUNDOS_FALA}s)
...
def corrigir_dialogos(roteiro, sinopse, idioma, problemas=None, elenco=None):
    ...
    system = f"""
Você é um roteirista especialista em diálogos para vídeos virais de {DURACAO_CENA} segundos.
...
- Total: {min_palavras}-{max_palavras} {unidade} por cena (~{MIN_SEGUNDOS_FALA}-{MAX_SEGUNDOS_FALA}s de fala)
- Pausa fixa {{"PAUSE": {PAUSA_PADRAO}}} entre cada fala
- Falas CURTAS ({min_linha}-{max_linha} {unidade} cada) — ritmo rápido estilo TikTok
...
Muitas falas curtas ({min_linha}-{max_linha} {unidade}). Preencha {MIN_SEGUNDOS_FALA}-{MAX_SEGUNDOS_FALA}s de áudio por cena.
NUNCA ultrapasse {MAX_SEGUNDOS_FALA}s nem fique abaixo de {MIN_SEGUNDOS_FALA}s.
"""
