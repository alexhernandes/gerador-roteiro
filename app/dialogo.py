...
PALAVRAS_POR_SEGUNDO = {
    ...
    "es": 3.0,
    "de": 2.7,
    # Japones usa caracteres falados por segundo, pois normalmente nao ha
...
MIN_SEGUNDOS_FALA = round(DURACAO_CENA * 0.75, 1)
MAX_SEGUNDOS_FALA = round(DURACAO_CENA - 0.8, 1)
PAUSA_PADRAO = 0.2
...
PADRAO_JAPONES = re.compile(r"[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff]")
...
def contar_unidades_texto(texto, idioma):
    """Conta palavras latinas ou caracteres japoneses para estimar o audio."""
    codigo = normalizar_idioma(idioma)
    if codigo == "ja":
        return len(PADRAO_JAPONES.findall(str(texto or "")))
    return len(str(texto or "").split())


def unidade_timing(idioma):
    return "caracteres japoneses" if normalizar_idioma(idioma) == "ja" else "palavras"


def limites_linha(idioma):
    if normalizar_idioma(idioma) == "ja":
        return 12, 24
    return 4, 12
...
def detectar_idioma_texto(texto):
    texto = str(texto or "")
    caracteres_japoneses = len(PADRAO_JAPONES.findall(texto))
    if caracteres_japoneses >= 2:
        return "ja"
...
def texto_no_idioma(texto, idioma_esperado):
    ...
    if detectado == "desconhecido":
        ...
        if idioma == "de" and re.search(
            ...
            return False
        if idioma == "ja":
            return len(PADRAO_JAPONES.findall(texto)) >= 2
        return True

    if idioma != "ja" and detectado == "ja":
        return False
...
def calcular_timing(dialogue_lines, idioma, duracao_cena=DURACAO_CENA):
    ...
    total_palavras = sum(contar_unidades_texto(t, codigo) for t in textos)
    total_pausas = sum(
        linha.get("PAUSE", 0)
        for linha in dialogue_lines
        if "PAUSE" in linha
...
def _segundos_das_falas(falas, wps, codigo):
    palavras = sum(contar_unidades_texto(f["TEXT"], codigo) for f in falas)
    pausas = max(0, len(falas) - 1) * PAUSA_PADRAO
    return palavras / wps + pausas


def _encurtar_texto(texto, codigo):
    if codigo != "ja":
        palavras = texto.split()
        return " ".join(palavras[:-1]) if len(palavras) > 1 else texto
    indices = [m.start() for m in PADRAO_JAPONES.finditer(texto)]
    if len(indices) <= 2:
        return texto
    indice = indices[-1]
    return (texto[:indice] + texto[indice + 1:]).strip()
...
def ajustar_timing_cena(cena, idioma):
    """Normaliza locks/pausas e corta excesso sem inventar falas novas."""
    ...
    def segundos():
        return _segundos_das_falas(falas, wps, codigo)
    ...
    while segundos() > MAX_SEGUNDOS_FALA and tentativas < 20:
        ...
        indice = max(
            candidatos,
            key=lambda i: contar_unidades_texto(falas[i]["TEXT"], codigo),
        )
        unidades = contar_unidades_texto(falas[indice]["TEXT"], codigo)
        if unidades > 2:
            falas[indice]["TEXT"] = _encurtar_texto(falas[indice]["TEXT"], codigo)
