...
def enriquecer_sinopse(sinopse, idioma):
    ...
    sinopse["ai_instructions"] = {
        ...
        "use_instead": {
            ...
            "scene_videos": f"roteiro.json → AI_VIDEO_TASK (one {DURACAO_CENA}s video per scene)",
...
def enriquecer_roteiro(roteiro, aspect_ratio, idioma, elenco=None, opcoes_agente=None):
    ...
    for chave in sorted(cenas, key=lambda k: cenas[k].get("SCENE_NUMBER", 0)):
        ...
        cena["AI_VIDEO_TASK"] = (
            ...
            f"Location lock: {cena.get('LOCATION_LOCK', '')[:300]}. "
            f"Continuity in: {cena.get('CONTINUITY_IN', '')[:300]}. "
            f"Continuity out: {cena.get('CONTINUITY_OUT', '')[:300]}. "
