...
def chave_speaker(nome):
    chave = re.sub(r"[^\w]+", "_", str(nome or "").upper(), flags=re.UNICODE).strip("_")
    return chave or "CHARACTER"


def _genero_canonico(gender):
    texto = str(gender or "").lower()
    if any(x in texto for x in ("female", "femin", "mulher", "girl", "woman", "女性", "女")):
        return "Strictly Female"
    if any(x in texto for x in ("male", "masc", "homem", "boy", "man", "男性", "男")):
