...
def problemas_continuidade(roteiro, sinopse):
    ...
    for cena in cenas_ordenadas:
        ...
        if not str(cena.get("LOCATION_LOCK", "")).strip():
            problemas.append(f"Cena {num}: sem LOCATION_LOCK para continuidade visual.")
        if not str(cena.get("CONTINUITY_IN", "")).strip():
            problemas.append(f"Cena {num}: sem CONTINUITY_IN.")
        if not str(cena.get("CONTINUITY_OUT", "")).strip():
            problemas.append(f"Cena {num}: sem CONTINUITY_OUT.")
    ...
    for anterior, atual in zip(cenas_ordenadas, cenas_ordenadas[1:]):
        ...
        saida = " ".join(str(anterior.get("CONTINUITY_OUT", "")).split()).casefold()
        entrada = " ".join(str(atual.get("CONTINUITY_IN", "")).split()).casefold()
        if saida and entrada and saida != entrada:
            problemas.append(
                f"Cena {num}: CONTINUITY_IN nao corresponde ao CONTINUITY_OUT anterior."
            )
